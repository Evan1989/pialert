...
class HTMLChart {
     ...
     * @return string
     */
    public function getDailyAlertsChart(PiAlertGroup|string $param, string $extSystem) : string {
        if ( $param instanceof PiAlertGroup) {
            $query = $param->getAlertCountForDiagram(ONE_MONTH);
        } else {
            $query = PiAlertGroup::getDailyAlertCountForDiagram($param, $extSystem, ONE_MONTH);
        }
        $data1= array();
     ...
     * @return string
     */
    public function getDailyAlertsPercentChart(PiAlertGroup|string $param, string $extSystem) : string {
        if ( $param instanceof PiAlertGroup) {
            $query = $param->getAlertPercentForDiagram(ONE_MONTH);
        } else {
     ...
     * @return string
     */
    public function getHourAlertsChart(string $piSystemName, string $extSystem) : string {
        $weekDay = date("N")-1; // от 0 до 6
        $hour = date("H");

        $query = PiAlertGroup::getHourAlertCountForDiagram($piSystemName, $extSystem);
        $data1= array();
        while($row = $query->fetch()) {
        ...
        foreach ( $systems as $system ) {
            $avgCounts = $alertAnalytics->getAverageAlertCounts($system, $extSystem);
            for ($i = 0; $i < 24; $i++) {
